...
inline void FreeCameraSystem(CameraComponent& cameraComp, TransformComponent& cameraTran, CameraMovementComponent& cameraMove, InputManagerPtr& input, float dt)
{
	...
	if (cameraComp.mouse_xy_prev.x >= 0 && mouse.leftButton)
	{
		...
		cameraComp.yaw -= deltaMouse.x * cameraMove.sensitivity;
		cameraComp.pitch -= deltaMouse.y * cameraMove.sensitivity;
	...
	cameraTran.pos += movement * cameraMove.moveSpeed * dt;
